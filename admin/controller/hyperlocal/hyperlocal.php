<?php
class ControllerHyperlocalHyperlocal extends Controller {
	...
	public function index() {
		...
		$this->data = array_merge($this->data,$this->load->language('hyperlocal/hyperlocal'));
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(isset($files['up_file']['tmp_name']) AND $files['up_file']['tmp_name']) {
				...
				if(isset($csv_extention[1]) AND $csv_extention[1] == 'csv') {
					...
					$this->response->redirect($this->url->link('hyperlocal/hyperlocal/matchdata', 'user_token=' . $this->session->data['user_token'], 'SSL'));
				} else {
      ...
      'separator' => false
   		...
   		$this->data['breadcrumbs'][] = array(
      'text'      => $this->language->get('wk_shippinglist'),
      'href'      => $this->url->link('hyperlocal/shippinglist', 'user_token=' . $this->session->data['user_token'], 'SSL'),
      'separator' => ' :: '
   		);

   		$this->data['breadcrumbs'][] = array(
      'text'      => $this->language->get('heading_title'),
	     'href'      => $this->url->link('hyperlocal/hyperlocal', 'user_token=' . $this->session->data['user_token'], 'SSL'),
      'separator' => ' :: '
   		);

		$this->data['back'] = $this->url->link('hyperlocal/hyperlocal', 'user_token=' . $this->session->data['user_token'], 'SSL');
		$this->data['action'] = $this->url->link('hyperlocal/hyperlocal', 'user_token=' . $this->session->data['user_token'], 'SSL');
		$this->data['header'] = $this->load->controller('common/header');
		$this->data['footer'] = $this->load->controller('common/footer');
		$this->data['column_left'] = $this->load->controller('common/column_left');
		$this->response->setOutput($this->load->view('hyperlocal/hyperlocal',$this->data));

	}

	public function matchdata() {

		$this->load->language('hyperlocal/hyperlocal');
	...
	public function stepTwo($fields = array()) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $fields == array()) {
			...
			if(isset($this->error['warning']) AND $this->error['warning']) {
				...
				$this->response->redirect($this->url->link('hyperlocal/shippinglist', 'user_token=' . $this->session->data['user_token'], 'SSL'));
      ...
      'separator' => ' :: '
   		...
   		$this->data['breadcrumbs'][] = array(
      'text'      => $this->language->get('heading_title') . ' ' . $this->language->get('heading_title_1'),
			   'href'      => $this->url->link('hyperlocal/hyperlocal', 'user_token=' . $this->session->data['user_token'], 'SSL'),
      'separator' => ' :: '
   		);

   		$this->data['breadcrumbs'][] = array(
      'text'      => $this->language->get('heading_title') . ' ' . $this->language->get('heading_title_2'),
			   'href'      => $this->url->link('hyperlocal/hyperlocal/matchdata', 'user_token=' . $this->session->data['user_token'], 'SSL'),
      'separator' => ' :: '
   		);
		...
		$this->data['action'] = $this->url->link('hyperlocal/hyperlocal/stepTwo', 'user_token=' . $this->session->data['user_token'], 'SSL');

		$this->data['cancel'] = $this->url->link('hyperlocal/hyperlocal', 'user_token=' . $this->session->data['user_token'], 'SSL');

		$this->data['header'] = $this->load->controller('common/header');
		$this->data['footer'] = $this->load->controller('common/footer');
		$this->data['column_left'] = $this->load->controller('common/column_left');

		$this->response->setOutput($this->load->view('hyperlocal/hyperlocalstep2',$this->data));
	...
	private function matchDataTwo() {
		...
		if(!isset($this->session->data['csv_file_shipping']))
			$this->response->redirect($this->url->link('hyperlocal/hyperlocal', 'user_token=' . $this->session->data['user_token'], 'SSL'));
	    ...
	    foreach ($shippingdatas as $newshipping) {
	    	...
	    	$result = $this->model_hyperlocal_hyperlocal->addShipping($newshipping);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'hyperlocal/hyperlocal')) {
