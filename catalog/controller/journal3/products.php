	...
	protected function beforeRender() {
		...
		foreach ($this->settings['items'] as $key => $item) {
			...
			foreach ($products as $product_id => $value) {
				if ($this->config->get('module_hyperlocal_status')  ) {
					$this->registry->set('hyperlocal',new Hyperlocal($this->registry));
						$product_ids_hyperlocal = $this->hyperlocal->getAllValidProducts();
			
						$f = 1 ;
						if ( isset($product_id) && in_array($product_id,explode(',', $product_ids_hyperlocal)))
					  {
					  $f= 0 ;
					  }
					if ($f) {
		...
		foreach ($results as $result) {
			...
			if ($this->config->get('module_hyperlocal_status')  ) {
				$this->registry->set('hyperlocal',new Hyperlocal($this->registry));
					...
					$f = 1 ;
					if ( isset($result['product_id']) && in_array($result['product_id'],explode(',', $product_ids_hyperlocal)))
				  {
				  $f= 0 ;
				  }
				if ($f) {
