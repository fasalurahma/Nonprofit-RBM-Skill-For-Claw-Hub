<?php
class ControllerAccountCustomerpartnerShippinglist extends Controller {
	...
	public function index() {
		...
		$data = array_merge($data,$this->language->load('account/customerpartner/shippinglist'));

		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/customerpartner/shippinglist', '', 'SSL');
		...
		$this->load->model('account/shippinglist');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if(isset($this->request->post['shipping_add_flatrate'])) {
           		...
           		$this->model_account_shippinglist->addFlatShipping($this->customer->getId(),$this->request->post['shipping_add_flatrate']);
           	}
			...
			if(isset($files['up_file']['tmp_name']) AND $files['up_file']['tmp_name']) {
				...
				if(isset($csv_extention[1]) AND $csv_extention[1] == 'csv') {
					...
					$this->response->redirect($this->url->link('account/customerpartner/shippinglist/matchdata', '', 'SSL'));
				} else {
				...
				$this->response->redirect($this->url->link('account/customerpartner/shippinglist', '', 'SSL'));
		...
		$results = $this->model_account_shippinglist->viewdata($filter_array);
		...
		$flatrate = $this->model_account_shippinglist->getFlatShipping($this->customer->getId());
		...
		if(isset($flatrate['amount'])) {
			...
			$data['shipping_hyperlocal_add_flatrate_amount'] = $flatrate['amount'];
		}
        ...
        'text'      => $this->language->get('heading_title_hyperlocal'),
			     'href'      => $this->url->link('account/customerpartner/shippinglist' . $url, '', 'SSL'),
        'separator' => $this->language->get('text_separator'),
      	);
		...
		$data['action'] = $this->url->link('account/customerpartner/shippinglist', '', 'SSL');

		$data['delete'] = $this->url->link('account/customerpartner/shippinglist/delete', '', 'SSL');

		$data['back'] = $this->url->link('account/account', '', 'SSL');

		$data['flatshipping'] = $this->language->get('flatshipping') . $this->config->get('shipping_hyperlocal_flatrate');
		...
		$data['sort_name'] = $this->url->link('account/customerpartner/shippinglist', '&sort=name' . $url);
		$data['sort_price'] = $this->url->link('account/customerpartner/shippinglist', '&sort=cs.price' . $url);
		$data['sort_range_to'] = $this->url->link('account/customerpartner/shippinglist', '&sort=cs.range_to' . $url);
		$data['sort_range_from'] = $this->url->link('account/customerpartner/shippinglist', '&sort=cs.range_from' . $url);
		$data['sort_weight_to'] = $this->url->link('account/customerpartner/shippinglist', '&sort=cs.weight_to' . $url);
		$data['sort_weight_from'] = $this->url->link('account/customerpartner/shippinglist', '&sort=cs.weight_from' . $url);
		...
		$pagination->url = $this->url->link('account/customerpartner/shippinglist', $url . '&page={page}');
		...
		$this->response->setOutput($this->load->view('account/customerpartner/shippinglist' , $data));

	}

	public function matchdata() {

		$this->load->language('account/customerpartner/shippinglist');
	...
	public function stepTwo($fields = array()) {
		...
		$data['heading_title_hyperlocal'] = $this->language->get('heading_title_hyperlocal') . $this->language->get('heading_title_2');
		...
		$data['action'] = $this->url->link('account/customerpartner/shippinglist/stepTwo', '', 'SSL');

		$data['cancel'] = $this->url->link('account/customerpartner/shippinglist', '', 'SSL');
		...
		$this->response->setOutput($this->load->view('account/customerpartner/shippinglist_next' , $data));
	...
	private function matchDataTwo() {
	    ...
	    foreach ($files as $line) {
	    	...
	    	if($entry) {
	    		$shipping_datas[$i] = array();
	    		foreach($post as $key=>$postchk) {
		    		$shipping_datas[$i][$key] = $line[$postchk];
	    		}
	    		$i++;
	    	}
	    ...
	    $updatechk = 0;
	    foreach ($shipping_datas as $new_shipping) {
	    	$result = $this->model_account_shippinglist->addShipping($new_shipping);
	    	if($result)
	    		$updatechk++;
	    }
	...
	public function delete() {
		...
		if (isset($this->request->post['selected'])) {
			foreach ($this->request->post['selected'] as $id) {
				$this->model_account_shippinglist->deleteentry($id);
	  		}

			$this->session->data['success'] = $this->language->get('text_success_delete');

			$this->response->redirect($this->url->link('account/customerpartner/shippinglist', '' . $url, 'SSL'));
		}
