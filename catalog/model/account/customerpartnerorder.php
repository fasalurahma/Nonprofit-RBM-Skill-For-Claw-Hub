	...
	public function customerpartner($order_info,$order_status, $comment = '', $order_status_id = 0){
		...
		if($this->config->get('marketplace_allowed_shipping_method')) {
			if(in_array($order_info['shipping_code'],$this->config->get('marketplace_allowed_shipping_method'))) {
				...
				$shipping = 0; // set a dafult value of 0
				if($sellers_count == 1 && isset($this->session->data['shipping_method']['cost'])) {
		...
		foreach ($order_product_query->rows as $product) {
            ...
            $shipping_applied = $shipping_quote['quote']['wk_custom_shipping']['cost'];
				...
				if(isset($mpSellers['customer_id']) && isset($this->session->data['seller_priority_shipping_sum']) && isset($this->session->data['seller_priority_shipping_sum'][$mpSellers['customer_id']])) {
					$shipping_applied += $this->session->data['seller_priority_shipping_sum'][$mpSellers['customer_id']];
					$commission_array['customer'] += $this->session->data['seller_priority_shipping_sum'][$mpSellers['customer_id']];
				}
			...
			// if (!$this->model_payment_amazon_checkout->isAmazonOrder($order_info['order_id'])) {
				...
				foreach ($mailToSellers as $value) {
					...
					if(isset($check_order_exists)){
									$mailData['mail_id']=$this->config->get('marketplace_mail_order_status_change')?$this->config->get('marketplace_mail_order_status_change'):$this->config->get('marketplace_mail_order');
							}
